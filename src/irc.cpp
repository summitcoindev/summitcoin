...
void ThreadIRCSeed()
{
    ...
    RenameThread("Summitcoin-ircseed");
...
void ThreadIRCSeed2()
{
	...
	int ntimeout = 1000;
        ...
        if (TestNet())
        {
            Send(hSocket, "JOIN #SummitcoincoinTEST\r");
            Send(hSocket, "WHO #SummitcoincoinTEST\r");
        } else
        {
	    LogPrintf("JOIN #Summitcoincoin\n");
            // randomly join #Summitcoincoin00-#Summitcoincoin05
            //int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #Summitcoincoin%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #Summitcoincoin%02d\r", channel_number).c_str());
        };
