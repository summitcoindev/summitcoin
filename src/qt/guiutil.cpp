...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(uri.scheme() != QString("Summitcoin"))
        return false;
...
{
    // Convert Summitcoin:// to Summitcoin:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("Summitcoin://"))
    {
        uri.replace(0, 12, "Summitcoin:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Summitcoin.lnk";
}
...
{
    return GetAutostartDir() / "Summitcoin.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("Summitcoin-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  Summitcoin-qt [" + tr("command-line options") + "]                     " + "\n";
    ...
    setWindowTitle(tr("Summitcoin-Qt"));
